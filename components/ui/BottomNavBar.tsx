...
import { Home, BarChart2, BookOpen, Calendar, Settings, Lock, LucideIcon } from 'lucide-react';
...
const NavItem: React.FC<NavItemProps> = ({ label, icon: Icon, isActive, onClick, isLocked = false }) => {
    return (
        <button
            onClick={onClick}
        ...
        >
            <div className="relative flex flex-col items-center">
                ...
                <div className="relative">
                    <Icon
                        className={`${isActive ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'} w-6 h-6 transition-colors`}
                        aria-hidden="true"
...
const BottomNavBar: React.FC<BottomNavBarProps> = ({ currentView, onNavigate, userTier }) => {
    ...
    return (
        <footer className="sticky bottom-0 bg-white/98 backdrop-blur-md p-1 border-t border-zinc-200/60 shadow-[0_-2px_10px_rgba(0,0,0,0.04)] z-50">
