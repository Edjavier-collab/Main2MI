...
/**
 * Dual-Run Analyze Session Edge Function
 * 
 * This function implements the dual-run comparison system for Strangler Fig migration.
 * 
 * Strategy:
 * 1. Call both legacy (analyze-session) and new (analyze-session-v2) functions
 ...
 * Returns: Feedback object (legacy format) + comparison metadata
 */

const FUNCTION_NAME = 'analyze-session';
const LEGACY_FUNCTION = 'analyze-session';
const NEW_FUNCTION = 'analyze-session-v2';
...
// Compare two feedback outputs (simplified version of BridgeAdapter logic)
...
function compareFeedbackOutputs(legacy: any, newOutput: any): DriftComparison {
  const differences: DriftComparison['differences'] = [];

  // Critical fields (must match exactly)
  const criticalFields = ['empathyScore', 'analysisStatus', 'skillsDetected'];
  ...
  const moderateFields = ['empathyBreakdown', 'whatWentRight', 'areasForGrowth', 'nextFocus'];
...
async function recordComparison(
  ...
  console.log(`[dual-run-analyze-session] Recorded comparison: semanticEqual=${comparison.semanticEqual}, consecutiveMatches=${newConsecutiveMatches}/${CUTOVER_THRESHOLD}`);
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      console.error('[dual-run-analyze-session] Missing Supabase environment variables');
    ...
    if (authError || !user) {
      console.error('[dual-run-analyze-session] Auth error:', authError);
      return errorResponse('Invalid or expired token. Please log in and try again.', 401, req);
    }

    const userId = user.id;
    console.log('[dual-run-analyze-session] Verified authenticated user:', userId.substring(0, 8) + '...');

    // Parse request body
    const requestBody = await req.json();
    const { transcript, patient } = requestBody;
    ...
    console.log('[dual-run-analyze-session] Calling legacy and new functions in parallel...');
    ...
    if (legacyResponse.status === 'fulfilled' && legacyResponse.value.ok) {
      ...
      console.error('[dual-run-analyze-session] Legacy function failed:', error);
      return errorResponse('Failed to analyze session', 500, req);
    }
    ...
    if (newResponse.status === 'fulfilled' && newResponse.value.ok) {
      ...
      console.warn('[dual-run-analyze-session] New function failed (non-critical):', error);
    ...
    if (newOutput) {
      comparison = compareFeedbackOutputs(legacyOutput, newOutput);
      console.log('[dual-run-analyze-session] Comparison result:', {
    ...
    try {
      ...
      console.error('[dual-run-analyze-session] Failed to record comparison:', dbError);
    ...
    console.error('[dual-run-analyze-session] Unexpected error:', error);
