...
/**
 * Coaching Summary Edge Function V2
 * 
 ...
 * For now, it's a copy of coaching-summary/index.ts to enable dual-run comparison.
 * 
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      console.error('[coaching-summary-v2] Missing Supabase environment variables');
    ...
    if (authError || !user) {
      console.error('[coaching-summary-v2] Auth error:', authError);
      return errorResponse('Invalid or expired token. Please log in and try again.', 401, req);
    }

    const userId = user.id;
    console.log('[coaching-summary-v2] Verified authenticated user:', userId.substring(0, 8) + '...');
    ...
    if (!geminiApiKey) {
      console.error('[coaching-summary-v2] GEMINI_API_KEY not configured');
    ...
    console.log('[coaching-summary-v2] Calling Gemini API for user:', userId.substring(0, 8) + '...', 'sessions:', sessionSummaries.length);
    ...
    try {
      ...
      if (error instanceof Error && error.message === 'Request timeout') {
        console.error('[coaching-summary-v2] Gemini API timeout after', RESPONSE_TIMEOUT_MS, 'ms');
    ...
    if (geminiResponse.status === 429) {
      console.error('[coaching-summary-v2] Gemini API rate limit exceeded');
    ...
    if (!geminiData.candidates || !geminiData.candidates[0] || !geminiData.candidates[0].content) {
      console.error('[coaching-summary-v2] Invalid Gemini API response structure:', JSON.stringify(geminiData, null, 2));
    ...
    try {
      ...
      console.error('[coaching-summary-v2] Failed to parse JSON response:', parseError);
    ...
    console.log('[coaching-summary-v2] Successfully generated coaching summary for user:', userId.substring(0, 8) + '...');

    return jsonResponse(normalizedSummary, 200, req);

  } catch (error) {
    console.error('[coaching-summary-v2] Unexpected error:', error);
